(function (window, undefined) {
    var
    pluginName = 'wangEditor', version = 'v1.0.0', updateTime = '2014/11/06', email = 'wangfupeng1988#163.com', i, item, idPrefix = 'wangeditor_' + Math.random().toString().replace('.', '') + '_', fontFamilyOptionsStr = '', fontFamilyOptions = ['宋体', '黑体', '楷体', '隶书', '幼圆', '微软雅黑', 'Arial', 'Verdana', 'Georgia', 'Times New Roman', 'Trebuchet MS', 'Courier New', 'Impact', 'Comic Sans MS'], colorOptionsStr = '', bgColorOptionsStr = '', colorOptions = { red: '红色', blue: '蓝色', green: '绿色', yellow: '黄色', black: '黑色', gray: '灰色', silver: '银色' }, fontsizeOptionsStr = '', fontsizeOptions = { 1: '10px', 2: '13px', 3: '16px', 4: '19px', 5: '22px', 6: '25px', 7: '28px' }, valueForLoop; for (item in fontsizeOptions) { if (Object.prototype.hasOwnProperty.call(fontsizeOptions, item)) { valueForLoop = fontsizeOptions[item]; fontsizeOptionsStr += '<li><a href="#" fontSize="' + item + '" style="font-size:' + valueForLoop + ';">' + valueForLoop + '</a></li>'; } }
    ...
    jQuery.fn.extend({
        wangEditor: function (options) {
            ...
            $menuContainer = $('<div></div>'), $menuToolbar = $('<div class="btn-toolbar"></div>'), $btnGroup = $('<div class="btn-group"></div>'), btnTemp = function (title, iconClass, isDropdown, modalTarget, btnContent) {
                var temp = '', btnClass = 'btn'; if (!title || typeof title !== 'string') { throw new Error('btnTemp：必须传入title参数，而且title必须是字符串类型！'); }
                ...
                if (!content || typeof content !== 'string') { throw new Error('dropdownMenuTemp：content参数不能为空，且必须为字符串类型！'); }
                ...
                if (!id || typeof id !== 'string') { throw new Error('modalTemp: id参数不能为空，且必须为字符串类型'); }
                if (!title || typeof title !== 'string') { throw new Error('modalTemp: title参数不能为空，且必须为字符串类型'); }
